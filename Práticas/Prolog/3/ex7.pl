...
short_classes(L) :-
    findall(UC-Day/Hour, (class(UC, _, Day, Hour, Duration), Duration < 2), L).

course_classes(Course, Classes) :-
    findall(Day/Hour-ClassType, class(Course, ClassType, Day, Hour, _), Classes).

courses(L) :-
    setof(Course, (CT,D,T,DT)^class(Course, CT, D, T, DT), L).
...
schedule :-
    setof(Day/Time/Duration/Course-ClassType,
          class(Course, ClassType, Day, Time, Duration), Classes),
    print_list(Classes).
